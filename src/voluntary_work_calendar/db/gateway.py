...
class Gateway(ABC):
    ...
    @abstractmethod
    def insert_presence(
        data: date, orario_from: str, orario_to: str, name: str
    ) -> bool:
        ...
...
class DBGateway(Gateway):
    ...
    def delete_presence(self, data: date, name: str) -> bool:
        ...
        if calendar_df.shape[0] > 0:
            ...
            st.warning(
                "Il volontario {} non ha inserito la presenza per il {}".format(
                    name, data
                )
            )
            return False
    ...
    def get_calendar(self) -> pd.DataFrame:
        calendar_df = get_table(db_session=get_db(), model=Calendar)
        return calendar_df
    ...
    def get_volunteers_name(self) -> List[str]:
        volunteers_df = get_table(db_session=get_db(), model=Volunteers)
    ...
    def insert_presence(
        self, data: date, orario_from: str, orario_to: str, name: str
    ) -> bool:
        create_table(
            db_session=get_db(),
            model=Calendar,
...
class CSVGateway(Gateway):
    ...
    def delete_presence(self, data: date, name: str) -> bool:
        ...
        if idx_to_delete.sum() == 0:
            ...
            df = df.drop(
                idx_to_delete.index[idx_to_delete == True], axis=0
    ...
    def delete_volunteer(self, name: str) -> bool:
        ...
        if idx_to_delete.sum() == 0:
            st.warning(
                "!!! Errore !!! Volontario non rimosso dalla lista perchè non presente"
            )
            return False
        else:
            df = df.drop(
    ...
    def insert_presence(
        ...
        row = pd.DataFrame(
            [[starting_id, data, orario_from, orario_to, name]], columns=fields
        )
        df = pd.concat([df, row], axis=0)
        df.to_csv(file_path, index=False)
        text = "Save {} in following path {}".format(filename, file_path)
    ...
    def insert_user(self, username: str, role: str, clean_password: str) -> bool:
        ...
        if not error_in_psw:
            row = pd.DataFrame(
                [[starting_id, username, role, get_hash(clean_password)]],
                columns=fields,
            )
            df = pd.concat([df, row], axis=0)
            df.to_csv(file_path, index=False)
