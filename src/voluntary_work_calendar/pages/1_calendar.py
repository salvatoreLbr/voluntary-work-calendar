...
from voluntary_work_calendar.utils import hide_sidebar, menu_button, show_available_volunteers
...
st.set_page_config(page_title="Calendario presenze",  initial_sidebar_state=st.session_state.sidebar_state)
...
if calendar_df.shape[0] == 0:
    st.warning("Non è stata inserita ancora nessuna presenza. \n Clicca su 'Inserisci presenza' per definire un calendario.")
else:
    with st.container():
        ...
        data_selected = st.date_input(
            ...
            key="selected_date",
            value=datetime.strptime(st.session_state["selected_date_str"], "%Y-%m-%d") if "selected_date_str" in st.session_state.keys() else datetime.today().date(),
            format="DD/MM/YYYY"
        )
        ...
        if "selected_date_str" in st.session_state.keys():
            if "calendar_df" in st.session_state.keys():
                ...
                if idx.sum()>0:
                    availables_volunteers = calendar_df.loc[idx, ["name", "orario_da", "orario_a"]].values
                    st.write("Volontari presenti il {}:".format(selected_date_str))
                    for v in availables_volunteers:
                        st.write("- {} sarà presente dalle {} alle {}".format(v[0], v[1], v[2]))
                else:
