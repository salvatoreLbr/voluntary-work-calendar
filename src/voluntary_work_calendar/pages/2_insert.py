...
#: Set page config
st.set_page_config(page_title="Calendario - inserisci presenza",  initial_sidebar_state=st.session_state.sidebar_state)
...
if len(volunteers_name_list) > 0:
    with st.form("inserisci_presenza_form"):
        ...
        with col_from:
            orario_from = st.time_input(
                label="Dalle",
                value=datetime.time(10,00)
            )
            orario_from_str = str(orario_from.hour)+":"+str(orario_from.minute).zfill(2)
        with col_to:
            orario_to = st.time_input(
                label="Alle",
                value=datetime.time(18,00)
            )
        ...
        if submit_button:
            if orario_to <= orario_from:
                st.warning("!!! Attenzione è stato selezionato un orario di fine turno anteriore all'orario di inizio turno !!!")
                raise Exception("!!! Attenzione è stato selezionato un orario di fine turno anteriore all'orario di inizio turno !!!")
    ...
    st.warning("Non c'è alcun volontario registrato. Inserisci almeno un volontario dal pannello 'Impostazioni' prima di aggiungere il suo turno.")
